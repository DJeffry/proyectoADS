...
namespace CapaNegocio
{
    public class storedProcedure
    {
        ...
        public bool pb(string[] obj, string nsp) //sirve para insertar, actualizar y borrar, devuelve true o false
        {
            ...
            try
            {
                SqlCommand comando = new SqlCommand("parvularia." + nsp, Conexion.cnn);//creamos el query, pero esta vez solo le enviaremos el procedimiento almacenado
                                          ...
                                          //obtenemos como parametro la tabla que queremos consultar
        {
            ...
            try
            {
                var select = "select * from parvularia." + tabla + ";";//creamos un query diciendole que seleccionaremos todos los datos de la tabla parametro
                                                           ...
                                                           //obtenemos como parametro la tabla que queremos consultar
        {
            ...
            try
            {
                var select = "select " + columnas + " from parvularia." + tabla + ";";//creamos un query diciendole que seleccionaremos todos los datos de la tabla parametro
                var dataAdapter = new SqlDataAdapter(select, Conexion.AbrirConexion());// creamos un data adapter
                var commandBuilder = new SqlCommandBuilder(dataAdapter);//construimos la consulta
                var ds = new DataSet();//creamos un nuevo dataset
                dataAdapter.Fill(ds);//seteamos los datos
        ...
        public DataTable dt(string[] obj, string nsp)//sirve para obtener una tabla de la base de datos con valores especificos
        {
            ...
            try
            {
                ...
                using (SqlCommand command = new SqlCommand("exec parvularia." + nsp + " " + sql + ";", Conexion.AbrirConexion())) //creamos la consulta con los parametros (sql) y el nombre del procedimiento almacenado
                {
                    SqlDataReader reader = command.ExecuteReader();//ejecutamos el reader
                    foreach (var data in reader)
                    {//por cada valor que contenga el reader lo almacenaremos en data
        ...
        public List<object[]> lt(string[] obj, string nsp) //retorna un objeto tipo lista
        {
            try
            {
                SqlCommand comando = new SqlCommand("parvularia." + nsp, Conexion.AbrirConexion());//creamos el query, pero esta vez solo le enviaremos el procedimiento almacenado
                comando.CommandType = CommandType.StoredProcedure;//especificamos que el comando con el query es de tipo procedimiento almacenado
                foreach (string p in obj)
                {//en obj vienen los parametros para enviar a nuestro procedimiento almacenado, cada parametro lo almacenaremos en p
                    string[] words = new string[2];//creamos un array para separar el nombre del parametro con el parametro en sí
                                                 ...
                                                 //obtenemos como parametro la tabla que queremos consultar
        {
            try
            {
                SqlCommand comando = new SqlCommand("select * from parvularia." + tabla, Conexion.AbrirConexion());//creamos el query, pero esta vez solo le enviaremos el procedimiento almacenado
                leer = comando.ExecuteReader();//ejecutamos el comando para obtener los datos del procedimiento almacenado
                                                                  ...
                                                                  //obtenemos como parametro la tabla que queremos consultar
        {
            try
            {
                SqlCommand comando = new SqlCommand("select " + columnas + " from parvularia." + tabla, Conexion.AbrirConexion());//creamos el query, pero esta vez solo le enviaremos el procedimiento almacenado
                leer = comando.ExecuteReader();//ejecutamos el comando para obtener los datos del procedimiento almacenado
